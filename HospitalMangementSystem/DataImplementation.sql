...
ALTER TABLE patient.patient ADD Age AS (dbo.cal_PatientAge(PatientID));
...
alter table patient.wardrooms add constraint ckIN check(dbo.InPatient_Constr(PatientId)=0);
...
ALTER TABLE patient.patient ADD Status AS (dbo.RoomAvailability(PatientID));
